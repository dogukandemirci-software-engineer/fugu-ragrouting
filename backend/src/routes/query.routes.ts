...
import { requireApiKey, requireAuthOrApiKey } from '../middlewares/api-key.middleware';
...
// Dashboard query explorer: JWT auth. Streaming also accepts an API key so
// the SDK can use it directly.
router.post('/execute', requireAuth, rateLimitMiddleware, validateDto(QueryDto), QueryController.execute);
router.post('/stream', requireAuthOrApiKey, rateLimitMiddleware, validateDto(QueryDto), QueryController.stream);
