...
export function TeamPage() {
  ...
  const { data, isLoading } = useListTeamQuery();
  ...
  const handleRemove = async (memberId: string, name: string) => {
    ...
    await remove(memberId).unwrap().catch(() => toast.error('Failed to remove member'));
  ...
  return (
    <div className="flex flex-col h-full">
              ...
              {members.map((member) => (
                <div key={member.id} className="flex items-center gap-4 py-4 px-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-body-sm font-medium text-on-surface">{member.full_name}</p>
                  ...
                  {member.user_id !== user?.id && member.role !== 'owner' && (
                    <Button variant="ghost" size="sm" onClick={() => handleRemove(member.user_id, member.full_name)}>
      ...
      <Modal open={inviteOpen} onClose={() => setInviteOpen(false)} title="Add team member" size="sm">
