// Adapted from MagicUI (https://magicui.design/docs/components/terminal), MIT licensed.
import {
  Children,
  createContext,
  useContext,
...
export const AnimatedSpan = ({ children, delay = 0, className, startOnView = false, ...props }: AnimatedSpanProps) => {
  ...
  return (
    <motion.div
      ...
      className={cn('grid text-[14px] font-normal tracking-tight whitespace-pre', className)}
      onAnimationComplete={() => {
        if (!sequence) return;
...
export const TypingAnimation = ({
  ...
  return (
    <MotionComponent ref={elementRef} className={cn('text-[14px] font-normal tracking-tight', className)} {...props}>
      {displayedText}
    </MotionComponent>
  );
};
...
export const Terminal = ({ children, className, sequence = true, startOnView = true }: TerminalProps) => {
  ...
  const content = (
    <div
      ref={containerRef}
      className={cn('border-outline-variant bg-ink-dark z-0 h-full max-h-[560px] w-full rounded-xl border', className)}
    >
      <div className="border-outline-variant flex flex-col gap-y-2 border-b p-4">
      ...
      <pre className="p-4 sm:p-5 overflow-x-auto">
        <code className="grid gap-y-2 whitespace-pre">{wrappedChildren}</code>
      </pre>
    </div>
  );
